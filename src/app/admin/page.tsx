...
export default async function AdminPage() {
  ...
  const [{ data: profesionales }, { data: organizaciones }] = await Promise.all([
    ...
    allRows(
      supabase
        .from("organizations")
        .select("id, name, kind, created_at, organization_access(status, granted_at, expires_at, note)")
  ...
  return (
    <div className="mx-auto flex max-w-4xl flex-col gap-8 p-6">
      <header className="flex flex-col gap-2">
        <p className="section-label">Solo administración</p>
        <h1 className="page-title flex items-center gap-3">
          <ShieldAlert size={22} strokeWidth={1.75} aria-hidden className="text-ink-3" />
          Administración de plataforma
        </h1>
        <p className="text-sm text-ink-2">
          Revisión de acreditaciones y acceso comercial. Desde aquí no se ve
          ningún expediente: un administrador de plataforma no está asignado a
          ninguno, y la RLS se lo negaría igual.
        </p>
      </header>
