/**
 * Generadores de los documentos de contabilidad: XLSX, CSV y PDF.
 *
 * Vivían dentro del route handler de `/pro/contabilidad/export`. Se extraen
 * aquí porque el expediente anual necesita los MISMOS documentos dentro de su
 * ZIP, y duplicarlos habría dejado dos fuentes de verdad para la misma
 * exportación: en cuanto una cambiara, el gestor recibiría dos versiones
 * distintas de las mismas cifras sin que nadie lo notara.
 */
...
export function periodoLabel(f: FiltroPeriodo): string {
...
export function buildXlsx(libros: Libro[], resumen: ResumenAnual, f: FiltroPeriodo): Buffer {
...
export function buildCsv(libros: Libro[], resumen: ResumenAnual, f: FiltroPeriodo): string {
  const bloques = [
...
// --- PDF (resumen legible) --------------------------------------------------
export async function buildPdf(resumen: ResumenAnual, f: FiltroPeriodo): Promise<Uint8Array> {
